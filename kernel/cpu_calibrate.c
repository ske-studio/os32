...
/*  **NP21/W でも同じ丸めは起きる** (実測 rounds = 16 / ticks = 5 なので     */
/*  1 周 ≒ 0.31 tick) が、そこでは elapsed が 1 以上になり、ずれは高々       */
/*  3 倍ほど。実機 266MHz は 1 周が 0.1 tick 未満で **0 に落ちる**ので、      */
/*  1/7〜1/13 という桁のずれになった。エミュレータでは症状が出ない。        */
