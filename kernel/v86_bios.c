...
int v86_bios_int18(u32 *regs)
{
    ...
    /*  出力: AH = 指定グループの8キーの押下状態 (bit=1で押下中)        */
    /*  PC9800Bible §2-5: BDA 0000:052Ah のキー押下テーブルを返す      */
    /*  OS32では kbd_key_pressed[] が同等のデータを保持                 */
    /* ================================================================ */
    case 0x04: {
        u8 group = (u8)(regs[V86_REG_EAX] & 0xFF);
        u8 state = 0;
        if (group < 16) {
            state = kbd_key_pressed[group];
        }
