...
| R6 | 同時に ready が複数居るときの選択は**決定的**かつ**有界**: turn は 1 ラウンドにつきアプリごと 1 回 (`turn_used`)、入力優先の据え置きは連続 `MA_INPUT_STREAK_MAX` (=4) 回まで。ready なアプリは**自分が park した `OP_WAIT`** から `MA_STARVE_BOUND` (= `(2N−2) × (STREAK_MAX+1)`、N=4 で **30**) 回の `OP_WAIT` 以内に走る (ラウンドをまたぐ待ちを含む)。順は 入力群 > 導出群、入力群の中はフォーカス優先、それ以外は `last_run` の次から ID 昇順の巡回。**ただし top-level にしか出来ない仕事 (`LAUNCH` の保留 = `launch_pending`) があれば、他の条件より先に譲る** | 票 D11 / D11-3 / D11-3a (独立レビュー 2026-09-10 の指摘 1 と [P2] 差し戻し、PM 受入 2026-09-11)、契約 T3 / U5 |
...
検査は 17 ケース・92 個の `check()`。1 つでも落ちれば非ゼロ終了する。
...
| 12 | `turn_used` (1 ラウンド 1 turn) と `input_streak` (連続据え置きの上限) を入れ、`ma_should_park` と `ma_pick` の**両方**を直す | GREEN | **0** — ALL PASS (**78 検査**)。B は 10 回目の `OP_WAIT` で走る |
| 13 | ケース 16 (独立レビュー [P2] 2 回目の反例、4 本構成) を**追加**。上限の定数は旧 (`MA_MAX_APPS × (STREAK+1)` = 20) のまま | RED | **4** (16b-16e)。16a (「A は必ず走る」) は**通る** — 無限待ちは回 12 で消えており、壊れているのは**上限の式**だけ、という切り分けがそのまま出た。実測 30 > 旧上限 20 |
| 14 | `MA_STARVE_BOUND` を `(2 × MA_MAX_APPS − 2) × (MA_INPUT_STREAK_MAX + 1)` = **30** へ再導出。**規則そのものは変えていない**。あわせてケース 15c を `N`=2 の上限 10 との**等号**検査に締め、15d を追加 | GREEN | **0** — ALL PASS (**84 検査**) |
| 15 | ケース 17 (票 D11-3 の PM 受入 2026-09-11「`LAUNCH` の保留があれば譲る」) を**追加**。`MaState.launch_pending` と `run_cross_round` の `launch_mode` 引数は足したが、`ma_should_park` は回 12 のまま | RED | **3** (17b, 17e, 17f)。保留を立てても park 判定は 0 のまま = 「アプリが 1 本走っている間は 2 本目が永久に立たない」の再現 |
| 16 | `ma_should_park` の**先頭** (走っている 1 本かの門番の直後、`other_ready` の走査より前) に `if (st->launch_pending) return 1;` を入れる | GREEN | **0** — ALL PASS (**92 検査**) |
...
回 15 の RED も同じ形で、3 個とも「起きるはずのことが起きない」(保留を立てても譲らない) を
捉えている。同ケースの 17a / 17c / 17d は**従来の振る舞いが変わっていない**ことを見る
否定形で、分岐を入れる前は自明に通る — 回 16 (GREEN) の後に**初めて**「新しい分岐が既存の
2 条件を壊していない」という意味を持つ。17g / 17h (上界 30) も同様で、回 15 の時点では
`launch_pending` が誰にも読まれていないため自明に通っていた。

回 11 と回 13 の RED は否定形ではない — 3 個とも「起きるはずのことが起きない」を
実際に捉えている (回 11: park 判定が 200 回中 0 回。回 13: 実測 30 回が旧上限 20 を破る)。
回 13 で 16a だけが通ったのは重要で、**壊れていたのは機構ではなく上限の式だけ**だった
ことを示している (規則は変えずに定数の導出だけを直した根拠)。回 9 の RED でも、否定形の 4 個 (`12d 導出群ではフォーカスを優先しない`
...
| 15 | **自作入力で park を回避できない** (独立レビュー [P2] 1 回目の反例、`N`=2): A が毎周自分に `Focus` を湧かせ (`wm.rs:969-985` → `input.rs:855-872`)、B は `Paint` 待ちで park 中。park は必ず起きる / B は必ず走る / B は `N`=2 の上限 **10 にちょうど届く** / 定数 `MA_STARVE_BOUND` の内側 | 票 D11-3a、受入 G9 |
| 16 | **ラウンドをまたぐ待ちの上限** (同 2 回目の反例、`N`=4): A が入力を消費して `Paint` を残し park、B/C/D は入力群を維持。A は必ず走る / 再導出した上限以内 / **ちょうど 30 に届く** (式がタイト) / フォーカスを B・D に置いても超えない | 票 D11-3a、受入 G9 |
| 17 | **top-level にしか出来ない仕事があれば譲る** (PM 受入 2026-09-11、K5b-W からの提案): `LAUNCH` が保留なら他に ready が居なくても・自分の入力の据え置き中でも譲る / 保留が消えれば従来どおり / 保留での park は `input_streak` を動かさない / 保留を 1 回で消費した経路の待ち回数は従来と同じ **30** / 保留が続いても上界 30 を超えない | 票 D11-3、受入 G9 |
...
- **上限の式が依る 3 つの前提**そのもの (起算点 = park した `OP_WAIT` / 継続 ready /
  アプリ集合が不変)。ケース 15・16 はこの前提を満たす構成を作って測っているだけで、
  前提が崩れる遷移 (途中で起動・終了・kill が入る) の待ち時間は測っていない。
  票 D11-3a に前提として明記した。
