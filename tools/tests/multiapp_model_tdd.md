...
| R6 | 同時に ready が複数居るときの選択は**決定的**かつ**有界**: turn は 1 ラウンドにつきアプリごと 1 回 (`turn_used`)、入力優先の据え置きは連続 `MA_INPUT_STREAK_MAX` (=4) 回まで。ready なアプリは**自分が park した `OP_WAIT`** から `MA_STARVE_BOUND` (= `(2N−2) × (STREAK_MAX+1)`、N=4 で **30**) 回の `OP_WAIT` 以内に走る (ラウンドをまたぐ待ちを含む)。順は 入力群 > 導出群、入力群の中はフォーカス優先、それ以外は `last_run` の次から ID 昇順の巡回 | 票 D11 / D11-3a (独立レビュー 2026-09-10 の指摘 1 と [P2] 差し戻し)、契約 T3 / U5 |
...
検査は 16 ケース・84 個の `check()`。1 つでも落ちれば非ゼロ終了する。
