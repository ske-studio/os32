...
| R6 | 同時に ready が複数居るときの選択は**決定的**かつ**有界**: turn は 1 ラウンドにつきアプリごと 1 回 (`turn_used`)、入力優先の据え置きは連続 `MA_INPUT_STREAK_MAX` (=4) 回まで。ready なアプリは必ず `MA_STARVE_BOUND` (= 4 × 5 = 20) 回の `OP_WAIT` 以内に走る。順は 入力群 > 導出群、入力群の中はフォーカス優先、それ以外は `last_run` の次から ID 昇順の巡回 | 票 D11 / D11-3a (独立レビュー 2026-09-10 の指摘 1 と [P2] 差し戻し)、契約 T3 / U5 |
...
検査は 15 ケース・78 個の `check()`。1 つでも落ちれば非ゼロ終了する。
...
| 11 | ケース 15 (独立レビュー [P2] の反例) を**追加**。規則は回 10 のまま | RED | **3** (15a-15c)。200 回の `OP_WAIT` で **park 判定 0 回**、`Paint` 待ちの B は一度も走らない — レビュアーの指摘の再現 |
| 12 | `turn_used` (1 ラウンド 1 turn) と `input_streak` (連続据え置きの上限) を入れ、`ma_should_park` と `ma_pick` の**両方**を直す | GREEN | **0** — ALL PASS (**78 検査**)。B は 10 回目の `OP_WAIT` で走る (上限 20 の内側) |
...
回 11 の RED は否定形ではない — 3 個とも「起きるはずのことが起きない」を
実際に捉えている (park 判定が 200 回中 0 回)。回 9 の RED でも、否定形の 4 個 (`12d 導出群ではフォーカスを優先しない`
...
| 15 | **自作入力で park を回避できない** (独立レビュー [P2] の反例): A が毎周自分に `Focus` を湧かせ (`wm.rs:969-985` → `input.rs:855-872`)、B は `Paint` 待ちで park 中。park は必ず起きる / B は必ず走る / B は `MA_STARVE_BOUND` = 20 回の `OP_WAIT` 以内に走る (実測 10) | 票 D11-3a、受入 G9 |

## 模型が**わざと**持っていないもの

- 時間・タイマ割込み・優先度・タイムスライス。R2 のとおり切替は park/resume の
  往復だけで、スケジューラは存在しない。持たせると「足場を作らない」という
  票の決めに反する。R6 の巡回と上限は**選ぶ順と譲る回数を決定的にするだけ**で、
  時間で横取りする仕組みではない — 上限に当たったアプリも、自分が `OP_WAIT` に
  入った地点で譲るのであって、走っている最中に取り上げられることは無い。
