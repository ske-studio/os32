...
**NP21/W でも丸めは起きるが、症状が出るほどではなかった。** 実測は
`rounds = 16 / ticks = 5` = **1 周 ≒ 0.31 tick (3.1ms)**。旧コードはこれを
`elapsed = 0 → 1` に丸めて 200,000 loops/tick としていたので、真値
(640,000) の **約 1/3.2**。実機 266MHz は 1 周 0.1 tick 未満で **1/7〜1/13**
まで落ちる。3 倍のずれでは `cpu_delay_us(5)` が 1.6µs になるだけで 9600 の
1 文字時間 (1.04ms) には遠く及ばず hlt に落ちなかったが、10 倍を超えると
予算そのものが 1 文字時間を割り込む — そこが分かれ目だった。
