...
FS ごとではなく **VFS 層で一括して埋める**。マウントは
`(FS, dev_type, dev_id)` で一意 (同じ組の二重マウントは上の修正で
`VFS_ERR_EXIST`) なので、`VFS_MOUNT_DEV_ENCODE(dev_type, dev_id) + 1` を
`st_dev` に使えば、生きているマウントの間で一意かつ非 0 になる。
...
- **同じ物理ディスク上の別パーティションは区別しない。**
  ...
  同一ディスクの 2 パーティションは同時にマウントできない。したがって
  `st_dev` を (種別, unit) 単位にしても、今の VFS で見える範囲では一意。
