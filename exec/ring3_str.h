...
/*                                                                          */
/*  安全性: 深さが 1 以上なのは、カーネルが WM のハンドラ / ポンプを同期的に  */
/*  実行しているあいだだけで、その間 CPL=3 のコードは 1 命令も走らない。      */
/*  KAPI に渡るポインタは WM が選んだもの (gshell は `arg` をポインタとして   */
/*  解釈しない — 入力は SHM のスロット経由) なので、アプリがこの窓を使って    */
/*  常駐側のメモリを書かせる経路は無い。longjmp で WM を抜けた (park / kill)  */
/*  ときの深さの立ち直しは exec/exec.c (ディスパッチャの入口で 0 に戻す)。    */
/*                                                                          */
