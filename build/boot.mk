...
BOOT_ASM_OBJ = boot/loader_hdd_new.o
BOOT_ALL_OBJ = $(BOOT_ASM_OBJ) $(BOOT_C_OBJ)

boot/loader_hdd_new.o: boot/loader_hdd_new.asm
	$(AS) -f elf32 -o $@ $<
...
boot/loader_hdd_new.elf: $(BOOT_ALL_OBJ)
	$(LD) -m elf_i386 -T boot/loader.ld -o $@ $^ \
		-L$(shell $(CC) -print-libgcc-file-name | xargs dirname) -lgcc

boot/loader_hdd_new.bin: boot/loader_hdd_new.elf
	$(OBJCOPY) -O binary $< $@
	@SIZE=$$(wc -c < $@); \
	if [ $$SIZE -gt 8192 ]; then \
		echo "ERROR: loader_hdd_new.bin is $$SIZE bytes (max 8192)"; \
		exit 1; \
	else \
		echo "loader_hdd_new.bin: $$SIZE / 8192 bytes"; \
	fi

# === 統合ターゲット ===
boot: $(BIN_STANDALONE) boot/loader_hdd_new.bin

# === クリーン ===
clean-boot:
	rm -f $(BIN_STANDALONE) $(BOOT_ALL_OBJ) boot/loader_hdd_new.elf boot/loader_hdd_new.bin
