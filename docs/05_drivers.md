...
から AH=30h の 640x480 の引数を決めて呼び、同じ AH=30h で元のモードへ戻す (戻れなければ
`pegc_restore_text_sync` が AH=31h で読んだ周波数の 400 ラインへ — 24kHz 固定にはしない)。その間、
09A8h・09A0h・60h〜7Ah の偶数・A0h〜A6h の偶数を捕まえて**記録してから実機へ幅どおりに**通し、OUT を畳まずに
512 件まで積む (溢れ・INS/OUTS・66h 付きの IN/OUT EAX・AH≠05h は失敗で、列は出さない)。通常の V86 の開始・終了
