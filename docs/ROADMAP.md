...
PCI の列挙もここで計画する (オンボード LAN・拡張カード・内蔵 Trident が同じ入口から
見えるようになる**土台**として)。Trident のバックエンドは**保留** — NP21/W の
`tgui9680.c` が結線されておらず、書いても検証できないため。

### メモリマップの全体再配置と C11 への移行 (ユーザー決裁 2026-09-17)

v3 で行う。**この 2 つは同時に動かさない** — どちらも全ファイルに触れるので、
壊れたときにどちらが原因か切り分けられなくなる。
