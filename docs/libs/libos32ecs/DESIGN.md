...
詳細な依存関係グラフについては、[依存関係定義（正本）](../libos32/DEPENDENCY.md) を参照してください。

- **直接依存先**: `libos32math`
- **協調関係について (DEP-2)**: `libos32ecs` は `libos32asset` や `libos32chem` などを直接の下流として依存しているわけではなく、ゲーム層においてこれらを協調・連携させるためのコンポーネント構造を提供する役割を持ちます。
- **入力非依存 (DEP-5)**: `libos32ecs` コアは `libos32input` に直接依存しません。入力処理はゲーム側の System が `libos32input` の状態を取得して ECS の Velocity コンポーネント等に反映する設計となっています。

**重要**: libos32ecs は**描画ライブラリ (libos32gfx, libos32tilemap) に依存しない**。
...
/* 
 * ゲーム固有/外部連携コンポーネント推奨ビット割当 (MIN-2)
 * 複数アダプタの併用時の衝突を防ぐための標準割当例：
 *
 *   #define COMP_TEXT       0x00000400u  /* bit 10: テキストエンジン (libos32text) */
 *   #define COMP_INVENTORY  0x00000800u  /* bit 11: インベントリ (libos32inv) */
 *   #define COMP_ECON       0x00001000u  /* bit 12: 経済エンジン (libos32econ) */
 *   #define COMP_QUEST      0x00002000u  /* bit 13: クエスト・シナリオ進行 */
 */
```
...
```c
/* ゲーム側の使用例 */
#define COMP_INVENTORY  0x0800  /* bit 11 */
#define COMP_QUEST      0x2000  /* bit 13 */
...
**libpyxel は廃止。** 新規ゲームは libos32ecs + libos32input + libos32tilemap を
...
| ドキュメント | 内容 |
|-------------|------|
| [DESIGN.md](../libos32input/DESIGN.md) | 入力連携 |
| [DESIGN.md](../libos32chem/DESIGN.md) | 化学ブリッジ |
| [DESIGN.md](../libos32math/DESIGN.md) | fix16_t |
