...
| libos32tilemap | タイルセット画像を直接読込 | 同じ画像の二重ロード検出なし |
...
- **直接依存先**: KernelAPI
- **位置づけ**: アセット管理用の最下層（L0 基盤）ライブラリとして動作します。
...
### Q: libos32map/libos32tilemap の内部I/Oを置き換えるか？
...
- [ ] libos32tilemap のタイルセット読込をlibos32asset経由に移行
...
| ドキュメント | 内容 |
|-------------|------|
| [DESIGN.md](../libos32chem/DESIGN.md) | libos32chem 設計書 (RAMキャッシュパターン参照) |
| [DESIGN.md](../libos32input/DESIGN.md) | libos32input 設計書 (同一アーキテクチャパターン) |
