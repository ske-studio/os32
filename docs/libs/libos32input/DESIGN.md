...
2. **入力コードの散在** — libpyxel の `pyxel_input.c` に入力管理が閉じ込められており、
   libos32tilemap 単体やGUIシェルからは再利用不可
...
- **直接依存先**: `libos32math`, KernelAPI
- **GUIライブラリとの関係 (CON-3)**: 現在実装されている Immediate Mode GUI ライブラリである `libos32ui` が本ライブラリを利用するほか、将来の v1.1 ロードマップに計画されている Win32API 風のイベントループ基盤 `libos32gui` にも入力イベントを供給する前提で設計されています。
...
| ドキュメント | 内容 |
|-------------|------|
| [DESIGN.md](../libos32math/DESIGN.md) | libos32math 設計書 (fix16_t 依存先) |
| [DESIGN.md](../libos32chem/DESIGN.md) | libos32chem 設計書 (同一アーキテクチャパターン) |
