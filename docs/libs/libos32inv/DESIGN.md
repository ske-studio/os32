...
### 1.3 経済シミュレーション (libos32econ) との責務境界 (DUP-1)

`libos32inv` はプレイヤー個人のインベントリ・所持品管理、装備スロット、およびプレイヤーが直接やり取りするショップUIでの売買を管理します。これに対し `libos32econ` は市場の価格変動、需給シミュレーション、NPCによる交易などのマクロ経済を管理します。
ショップにおけるアイテムの最終取引価格は `libos32econ` が算出し、`libos32inv` はその算出価格を参照してプレイヤーのウォレットとの決済を行います。
 ...
 * (注: 重複排除のため、内部的に libos32ai の ai_weighted_pick を使用します, DUP-2)
 */
