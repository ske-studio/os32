...
### 1.1 なぜ libos32econ が必要か

ゲーム内の経済変動やNPCによる自律取引などのマクロ経済シミュレーションを、個々のゲームロジックに密結合させずに一元管理し、プレイヤーのアクションによる物価変動やインフレなどの複雑な創発的ゲームプレイを提供するためです。
...
- **直接依存先**: `libos32math`, `libos32db`

`libos32econ` は **描画 (gfx) には依存しない**。
経済シミュレーションとデータ管理のみを担当し、UIはゲーム側が担当する。

### libos32invとの責務境界と連携 (DUP-1)

`libos32econ` は世界経済・市場価格・NPC商人・マクロ需給をシミュレーションします。一方、`libos32inv` はプレイヤー個人の所持品・装備・店UIでの売買決済を管理します。
店頭での取引価格は `libos32econ` が決定し、`libos32inv` のショップ処理がそれを参照して取引を行います。
また、クラフトレシピについては、`libos32inv` (items.db `recipes`) はプレイヤー個人向けの簡易合成・クラフトを、`libos32econ` (econ.db `recipes`) は都市や市場に流通するマクロ生産シミュレーションとしてのクラフトをそれぞれ担当し、棲み分けを行います。
...
libos32chemと同じ層。アダプタ方式で疎結合:

> [!NOTE]
> **実装状況について**: 連携対象である `libos32ecs` は現在設計フェーズ（実装コード未着手）です。
...
```c
/* 引数 inventory には libos32inv の InvBag * を渡して呼び出します (DUP-1) */
...
| ドキュメント | 内容 |
|-------------|------|
| [DESIGN.md](../libos32chem/DESIGN.md) | libos32chem 設計書 (同層ライブラリ) |
| [DESIGN.md](../libos32math/DESIGN.md) | libos32math 設計書 (依存先) |
