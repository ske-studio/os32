...
### 1.1 なぜ libos32chem が必要か

化学反応や熱伝導など、オブジェクト間の相互作用ロジックを各ゲームで個別にハードコーディングすると、ルールの追加・変更のたびに再ビルドが必要になり開発効率が低下します。汎用的な化学エンジンを提供することで、データ駆動での創発的ゲームプレイを実現します。
...
- **描画非依存**: `libos32chem` は描画（`libos32gfx`）には依存しません。
...
### 2.1 属性 (Element) — 共有定数（DUP-3）

`libos32chem` と `libos32battle` で共通利用される属性（Element）ビットフラグは、プロジェクトの定数3層管理ルールに基づき、Shared Layer である `include/os32_kapi_shared.h` に定義されます。

```c
/* include/os32_kapi_shared.h 内の定義を参照します */
 ...
 * (注: 将来的には libos32asset の Phase 3 集約計画に基づき、内部 I/O およびキャッシュ処理が libos32asset 経由に置き換わる予定です, MIN-3) */
...
- [x] 伝播制御 (SPREAD アクションの再帰深度制限) — `CHEM_SPREAD_DEPTH=3`
- [x] デモプログラム (`chem_demo.c`: グリッドUI + インタラクティブ操作)

### Phase 4: ゲーム統合 *(保留 — 他機能が揃ってから着手)*

- [ ] libos32tilemap との連携設計
