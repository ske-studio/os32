...
- **連携について (DEP-1)**: `libos32ai` と `libos32board` の間に直接の依存関係はありません。AIが行動判断の入力元としてボード情報を使用する場合、上位のゲーム本体が双方のデータを読み出して中介します。
...
| ドキュメント | 内容 |
|-------------|------|
| [DESIGN.md](../libos32math/DESIGN.md) | libos32math 設計書 (乱数依存先) |
| [DESIGN.md](../libos32battle/DESIGN.md) | バトルエンジン (AI判断の消費者) |
| [DESIGN.md](../libos32board/DESIGN.md) | ボードエンジン (方向選択の消費者) |
