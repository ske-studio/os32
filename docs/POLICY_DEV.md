...
AIによるGit変更操作（`git add` / `git commit` 等）は、Windowsネイティブ環境（PowerShell）から直接実行してよい。
ただしマイクロコミットの単位（上記）と Conventional Commits を遵守し、`git push` 等のリモート操作はユーザーの指示があった場合のみ実行する。

> 旧ルール（WSL環境でのAI直接操作禁止）は、実行環境のWindowsネイティブ移行により前提が崩壊したため 2026-06-10 に削除（[POLICY_AI.md](POLICY_AI.md) §5 の運用例）。
