...
- **システムに付随する機能を上位メモリ (物理末尾側) に置かない** (2026-09-09 方針)。
  PC-98 の歴史上、16MB 付近に装置由来の予約帯ができるのは避けられないが、
  OS 側の都合で作る予約をそこへ足さない。物理末尾は素直に RAM のまま残し、
  アプリ帯 (CPL=3 は 0x500000〜0x800000 固定) と競合させない。
  この方針でホットデプロイの物理末尾 256KB 窓を撤去した — 窓は CPL=3 スタック帯と
  同じ範囲で、8MB 構成では必ず衝突していた。ユーザーランドの配送は HostDrv
  (`make deploy` → ゲストの `hsync`) に一本化した。
  例外は装置が実際に占める帯 (PEGC の 0xF00000、Cirrus の 0x1000000) で、
  これらは装置側の事実なので `physmem` のモデルで MMIO として扱う。
