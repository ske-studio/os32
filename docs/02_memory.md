...
```
__bss_end      = 0x16C9E0   (カーネル本体 434.5KB)
...
0x100000 - 0x16C9DF 434.5KB  カーネル .text/.data/.bss  (kernel.map の __bss_end まで)     RW
0x16C9E0 - 0x16CFFF 1.5KB    空き
...
**カーネル本体の予算**: 468KB 中 434.5KB を使用 (残り 33.5KB)。

**カーネルがあと何 KB 育つと何が壊れるか** (`__bss_end` が伸びると `KHEAP_BASE` 以降が芋づるで動く)

- `__bss_end` +1.5KB で KHEAP_BASE が 1 ページ上がる。0x16D000 → 0x16E000。以降の KAPI / SHM / ガードが全部 4KB 動く
- `__bss_end` +33.5KB で **build/os32.ld の ASSERT がリンクを止める** (予算 MEM_KERNEL_IMAGE_MAX 超過)。止めるのが目的。超えたぶんだけ SHM 帯が カーネル帯域 0x1FFFFF を突き抜ける
