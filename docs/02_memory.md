...
```
__bss_end      = 0x16E260   (カーネル本体 440.6KB)
...
0x100000 - 0x16E25F 440.6KB  カーネル .text/.data/.bss  (kernel.map の __bss_end まで)     RW
0x16E260 - 0x16EFFF 3.4KB    空き
0x16F000 - 0x1BEFFF 320KB    カーネルヒープ (kmalloc)  (__bss_end を 4KB に切り上げた位置から) RW
0x1BF000 - 0x1BFFFF 4KB      KernelAPI テーブル  (KAPI_ADDR)                               RW
0x1C0000 - 0x1C0FFF 4KB      SHM 前方ガード                                                NP
0x1C1000 - 0x1F8FFF 224KB    共有メモリ本体  (16KB x SHM_BLOCK_COUNT。CPL=3 アプリの起動時に USER へ昇格 (exec.c、PDE 0 は全 PD 共有)) RW
0x1E9000 - 0x1F8FFF 64KB     GUI 予約 (末尾 4 ブロック)  (契約 T2。SDK の GUI_SHM_OFFSET = MEM_SHM_GUI_OFFSET) RW
...
**カーネル本体の予算**: 468KB 中 440.6KB を使用 (残り 27.4KB)。

**カーネルがあと何 KB 育つと何が壊れるか** (`__bss_end` が伸びると `KHEAP_BASE` 以降が芋づるで動く)

- `__bss_end` +3.4KB で KHEAP_BASE が 1 ページ上がる。0x16F000 → 0x170000。以降の KAPI / SHM / ガードが全部 4KB 動く
- `__bss_end` +27.4KB で **build/os32.ld の ASSERT がリンクを止める** (予算 MEM_KERNEL_IMAGE_MAX 超過)。止めるのが目的。超えたぶんだけ SHM 帯が カーネル帯域 0x1FFFFF を突き抜ける
