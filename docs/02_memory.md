...
```
__bss_end      = 0x18BA40   (カーネル本体 558.6KB)
...
0x100000 - 0x18BA3F 558.6KB  カーネル .text/.data/.bss  (kernel.map の __bss_end まで)     RW
0x18BA40 - 0x18BFFF 1.4KB    空き
...
**カーネル本体の予算**: 596KB 中 558.6KB を使用 (残り 37.4KB)。

**カーネルがあと何 KB 育つと何が壊れるか** (`__bss_end` が伸びると `KHEAP_BASE` 以降が芋づるで動く)

- `__bss_end` +1.4KB で KHEAP_BASE が 1 ページ上がる。0x18C000 → 0x18D000。以降の KAPI / SHM / ガードが全部 4KB 動く
- `__bss_end` +37.4KB で **build/os32.ld の ASSERT がリンクを止める** (予算 MEM_KERNEL_IMAGE_MAX 超過)。止めるのが目的。超えたぶんだけ SHM 帯が カーネル帯域 0x1FFFFF を突き抜ける
