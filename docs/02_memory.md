...
```
__bss_end      = 0x1770A0   (カーネル本体 476.2KB)
...
0x100000 - 0x17709F 476.2KB  カーネル .text/.data/.bss  (kernel.map の __bss_end まで)     RW
0x1770A0 - 0x177FFF 3.8KB    空き
...
**カーネル本体の予算**: 596KB 中 476.2KB を使用 (残り 119.8KB)。

**カーネルがあと何 KB 育つと何が壊れるか** (`__bss_end` が伸びると `KHEAP_BASE` 以降が芋づるで動く)

- `__bss_end` +3.8KB で KHEAP_BASE が 1 ページ上がる。0x178000 → 0x179000。以降の KAPI / SHM / ガードが全部 4KB 動く
- `__bss_end` +119.8KB で **build/os32.ld の ASSERT がリンクを止める** (予算 MEM_KERNEL_IMAGE_MAX 超過)。止めるのが目的。超えたぶんだけ SHM 帯が カーネル帯域 0x1FFFFF を突き抜ける
