...
```
__bss_end      = 0x18DBA0   (カーネル本体 566.9KB)
...
0x100000 - 0x18DB9F 566.9KB  カーネル .text/.data/.bss  (kernel.map の __bss_end まで)     RW
0x18DBA0 - 0x18DFFF 1.1KB    空き
...
**カーネル本体の予算**: 596KB 中 566.9KB を使用 (残り 29.1KB)。

**カーネルがあと何 KB 育つと何が壊れるか** (`__bss_end` が伸びると `KHEAP_BASE` 以降が芋づるで動く)

- `__bss_end` +1.1KB で KHEAP_BASE が 1 ページ上がる。0x18E000 → 0x18F000。以降の KAPI / SHM / ガードが全部 4KB 動く
- `__bss_end` +29.1KB で **build/os32.ld の ASSERT がリンクを止める** (予算 MEM_KERNEL_IMAGE_MAX 超過)。止めるのが目的。超えたぶんだけ SHM 帯が カーネル帯域 0x1FFFFF を突き抜ける
