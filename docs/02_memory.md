...
```
__bss_end      = 0x18DBE0   (カーネル本体 567.0KB)
...
0x100000 - 0x18DBDF 567.0KB  カーネル .text/.data/.bss  (kernel.map の __bss_end まで)     RW
0x18DBE0 - 0x18DFFF 1.0KB    空き
...
**カーネル本体の予算**: 596KB 中 567.0KB を使用 (残り 29.0KB)。

**カーネルがあと何 KB 育つと何が壊れるか** (`__bss_end` が伸びると `KHEAP_BASE` 以降が芋づるで動く)

- `__bss_end` +1.0KB で KHEAP_BASE が 1 ページ上がる。0x18E000 → 0x18F000。以降の KAPI / SHM / ガードが全部 4KB 動く
- `__bss_end` +29.0KB で **build/os32.ld の ASSERT がリンクを止める** (予算 MEM_KERNEL_IMAGE_MAX 超過)。止めるのが目的。超えたぶんだけ SHM 帯が カーネル帯域 0x1FFFFF を突き抜ける
