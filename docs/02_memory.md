...
```
__bss_end      = 0x172B60   (カーネル本体 458.8KB)
...
0x100000 - 0x172B5F 458.8KB  カーネル .text/.data/.bss  (kernel.map の __bss_end まで)     RW
0x172B60 - 0x172FFF 1.2KB    空き
...
**カーネル本体の予算**: 468KB 中 458.8KB を使用 (残り 9.2KB)。

**カーネルがあと何 KB 育つと何が壊れるか** (`__bss_end` が伸びると `KHEAP_BASE` 以降が芋づるで動く)

- `__bss_end` +1.2KB で KHEAP_BASE が 1 ページ上がる。0x173000 → 0x174000。以降の KAPI / SHM / ガードが全部 4KB 動く
- `__bss_end` +9.2KB で **build/os32.ld の ASSERT がリンクを止める** (予算 MEM_KERNEL_IMAGE_MAX 超過)。止めるのが目的。超えたぶんだけ SHM 帯が カーネル帯域 0x1FFFFF を突き抜ける
