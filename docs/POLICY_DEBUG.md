...
- **アイドル時の pgalloc (9MB)**: `limit_pfn=generic_end=2304`、`total_pages=1278`、
  ...
  取り、残る A/B 穴は [0x7BD000, 0x8BD000) の 256 ページ。**この穴が `0x800000`
  (= `MEM_APP_BAND_TOP`、アプリ固有 PDE の境界) をまたぐのは 9MB 付近だけ**。
  8MB では穴が境界の下、15MB では上に収まる。関連を疑う根拠はここまでで、**未確認**。
- **恐らく既存の不具合**: ホットデプロイ窓があった頃も 9MB の `mem_end` は 0x8C0000 で、
  A/B 穴 [0x77F000, 0x87F000) はやはり境界をまたぐ。窓の撤去 (2026-09-09) で
  生じたものではない、と考えているが**実際に旧カーネルで再現はしていない**。
