...
### 4-28. 9MB 構成で `v86 -t` が落ちる (未解決、2026-09-10。推測の 1 つを 2026-09-16 に撤回)
...
- **切り分けの手** (未実施): 失敗の記録は残る。`v86 -t` の後に `v86_smoke_result`
  (`kernel.map` の番地) を読み、**`0x8001` なら確保の失敗**、そうでなければ
  `v86_mem_setup()` に到達する前の別の問題。14MB (`ExMemory=13`) では `result: OK` /
  結果語 0 を確認済み (2026-09-16)。9MB の再現には `*.ini` の変更 ([D2]) が要る。
