...
### 4-9. ext2 ダブル間接ブロック (DIND) バッファ競合

- **現象**: `ext2_write_stream` / `ext2_read_file` で 270KB 超のファイルを操作するとデータが破損する。SQLite バイナリ (372KB) の統合時に発覚
- **原因**: `ext2_g_aux` グローバル共有バッファが、`ext2_bmap` (DINDテーブル読み込み) と `ext2_alloc_block` (ビットマップ操作) と `ext2_bmap_set` (DINDテーブル書き込み) の間で上書き競合を起こしていた。また、DIND インデックス計算 (`ext2_bmap` の `dind_offset / ptrs_per_block`) にも誤りがあった
- **対策**:
  - FSドライバのグローバル共有バッファは、1関数内で複数回の用途に使用しないこと
  - 270KB (= 12直接 + 256間接 = 268ブロック) を超えるファイルは DIND パスを通る。大規模ファイルテストを必ず実施すること
  - 読み込みバグを修正しても、元データの書き込みが不正なら意味がない。読み書きは対称的にテストすること
