...
(`ime` は FEP の有効化/辞書操作、`v86` は V86 モードでのゲスト起動、`cfg` は設定レジストリ `/etc/settings.db` の get / set / list / status / init / export — [archive/settings/TASK_S2.md](archive/settings/TASK_S2.md) §2)
