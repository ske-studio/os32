...
| [tasks/tools/TASK_KEY_INJECT.md](tasks/tools/TASK_KEY_INJECT.md) | キー注入で任意のバイトを送る **計画 (2026-09-17)** — `/api/key` の `text=ABC` が `abc` になる (変換関数が小文字に畳んでから needshift を 0 に固定)。**検証の側の穴**で OS32 本体は変えない。直す先は NP21/W と gui_gate.py。エディタの受入 E2〜E7 がこれで止まっている |
