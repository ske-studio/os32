...
[v3 計画書](tasks/v3/PLAN.md) は追記中の草案。[移行課題台帳](tasks/v3/MIGRATION_AUDIT.md) は現行文書・実装の確認結果、[文書の継承・分離マップ](tasks/v3/DOCUMENT_MAP.md) は参照先の整理。現行の C89 規約や最低動作条件は、この計画の追加だけでは変更しない。
