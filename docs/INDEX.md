...
| [KAPI_SPEC.md](KAPI_SPEC.md) | KernelAPI v66 仕様書 — 304 エントリの表 (ヘッダ 2 + 関数表の容量 300 (実装 238) + データフィールド 2、データ欄は 0x4B8 に固定) + API追加手順 |
...
| [tasks/tools/TASK_CHECK_MUT_PARALLEL.md](tasks/tools/TASK_CHECK_MUT_PARALLEL.md) | `make check` の変異段 (check-mut、14 本を `-j1`) を写しの上で変異する作りに移して並列にする **票 (未着手、v2.1 の後)** |
