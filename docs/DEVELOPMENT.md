...
|--------|-----------|------------|---------------|
| 400ラインモード | 640×400 | `gfx_init()` | 無効 (VSYNCポーリング同期) |
| 200ラインモード | 640×200 | `gfx_init_200()` | **自動有効** (ポート A4H/A6H) |

### ページフリッピング (200ラインモード)

200ラインモードでは各VRAMプレーンが16KB (32KB中の半分) しか使用しないため、
ハードウェアが提供する2ページのVRAMを活用する。
...
- **ラスタパレットとの排他**: `gfx_present_raster()` は走査中のパレット操作のため、
  フリップモードでは `gfx_present_dirty()` にフォールバックする。
...
*OS32 Technical Guide — Updated: 2026-04-21*
