...
- **HostDrvFS**: NP21/W HostDrv hypercall経由のホストファイルシステムアクセス。セッションベースI/O、volatile同期。※ 大容量ファイル同期の安定性向上のため、`hsync` コマンドは非推奨となり、ホストからのデプロイはWSL上のNHD直接同期（マウントコピー）に移行しています。
...
- **共有メモリ (SHM) の動的アドレス解決**: ユーザー空間ライブラリ（`programs/libos32db/` 等）における共有メモリのアドレスは、`KAPI` ポインタを基準として動的に解決します。具体的には `DB_SHM_PTR` を `((u8 *)kapi + 0x2000)` にキャストして参照します。これによりカーネルのメモリレイアウト変更に対して自動追従し、メモリの不整合による `#PF` (Page Fault) を防ぎます。
