...
カーネルを 0x110000 (1MB+64KB) に移動し、カーネル+ヒープ+KAPI+SHMを
1MB帯域に統合。外部プログラムのロードアドレス (0x400000) は変更しない。

## 設計方針

- カーネル帯域 (0x110000-0x1FFFFF): カーネル+ヒープ+KAPI+SHMを統合
...
|------|-----------|-----------|---------|------|
| カーネル本体 | 0x09000 | 0x110000 | 960KB | 変更 (0x100000-0x10FFFFはV86 A20用に予約) |
...
## カーネル帯域レイアウト (0x110000-0x1FFFFF)

カーネルBSS末尾 (__bss_end) から動的にヒープ等を配置:

```
0x110000           カーネル .text+.data+.bss (現在 ~200KB)
...
現在の __bss_end = 0x141E60 の場合の具体的配置:

```
0x110000-0x141E5F  カーネル code+data+bss (~200KB)
0x142000-0x191FFF  カーネルヒープ (320KB)
0x192000-0x192FFF  KAPIテーブル (4KB)
0x193000-0x193FFF  SHM前方ガード (NP, 4KB)
0x194000-0x1D3FFF  共有メモリ本体 (256KB)
0x1D4000-0x1D4FFF  SHM後方ガード (NP, 4KB)
0x1D5000-0x1FFFFF  空き/予約 (~170KB)
```
...
```c
#define KERNEL_LOAD_ADDR      0x110000UL  /* 旧: 0x9000 */
...
# kentry が 0x110000、__sqlite_start が 0x200000 であることを確認
```
