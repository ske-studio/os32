...
- リンカがエントリポイントを 0x110000 に解決するため、コード変更は不要
...
- SHMがカーネル帯域 (0x110000-0x1FFFFF) 内に移動するため、
  アドレス定数のみ変更。ロジック変更不要。
...
```asm
;; Phase 1: カーネルロード先を 0x110000 に変更
mov     edi, 00110000h       ;; 旧: 00009000h

;; Phase 2: SQLiteロード先を 0x200000 に変更
mov     edi, 00200000h       ;; 旧: 0018A000h

;; カーネルジャンプ先を 0x110000 に変更
db      0EAh
dd      00110000h            ;; 旧: 00009000h
dw      0008h
```
...
- カーネルロード先: 0x110000 (PM移行後にコピー or 直接ロード)
- カーネルジャンプ先: 0x110000

注意: FDDローダーはリアルモードで 0:9000h に読み込み後PMに遷移する。
PM移行後に 0x9000 → 0x110000 にコピーする処理を追加。
