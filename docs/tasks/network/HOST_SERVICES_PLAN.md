...
発行: PM (2026-09-13) / 状態: **計画 (詳細案、ユーザー決裁待ちの項目は §9)**。
...
要求は REQUEST フレームの payload (ASCII 行、最大約 1.4KB)、応答は RESPONSE (`status u16` + `len u32`) + 本文の L2 ストリーム。**OS32 → ホスト方向の大きなデータ** (印刷ジョブ本文) は、ホストに余裕があるので credit 不要 (LINK_PLAN §2 の非対称) — `PUT` 系は REQUEST フレームを複数に分けて送る (1 フレーム ≤ 1400B、`job` 番号 + `seq`)。

| 要求 | 引数 | 応答 | 用途 / 備考 |
|---|---|---|---|
| `GET <url>` | `http://` `https://` | status = HTTP ステータス、本文 | 既存。リダイレクトはホストが追う。ヘッダは返さない (v1) |
| `GET /file/<path>` | ホスト側パス | 200 / 404、本文 | 既存。**許可リスト** (§6) の下だけ |
| `TIME` | — | `YYYY-MM-DD HH:MM:SS` | 既存。時刻同期 (`date -sync` を足す) |
...
| `PRINT DATA <id> <seq>` + 本文 | ≤ 1400B / フレーム | 200 / 409 (seq 欠落) | ジョブ本文の分割送信。ホストはスプールに追記 |
...
| `CLIP PUT` + 本文 | ≤ 4KB (v1) | 200 | OS32 側のテキストをホストのクリップボードへ |
| `PUT /file/<path>` + 本文 | 許可リスト内 | 200 / 403 | **新規 (任意)**。ホストへのファイル書き出し (ログ、スクリーンショット)。分割は PRINT DATA と同じ |

`GET` と `TIME` は既存のまま。要求名は大文字、引数は空白区切り、本文が要る要求は `DATA` 系で分割する — 新しい要求を足すときは Host Agent の `_service_*` を 1 本増やすだけで、OS32 側 (KAPI) は変わらない。

## 3. KAPI (v51、末尾追記、[ABI1〜3])

LINK_PLAN §5-1 の 4 本を、GUI の協調 (`sys_halt` / `OP_WAIT` / `sys_yield` と共存する**非ブロッキング**) で確定する。第 5 の `host_write` を追加する (印刷 / CLIP PUT / PUT の本文送信用)。

| slot | name | args | ret | 規則 |
|---|---|---|---|---|
| 208 | `host_open` | `const char *req, u32 len` | h ≥ 0 / 負 | 要求行 (1〜1400B、CPL=3 ポインタは v50 と同じ範囲検証) を REQUEST で送る。**送るだけで戻る** (応答を待たない)。同時ハンドルは **2** (ストリームは 1 本なので本文を持てるのは 1 つ、もう 1 つは応答待ち用)。リンク未確立 (HELLO 前) は `OS32_ERR_IO`、ドライバ無効は `OS32_ERR_NOSYS` |
| 209 | `host_status` | `i32 h, u32 *status, u32 *length` | 0 / `OS32_ERR_AGAIN` / 負 | RESPONSE 未着なら **AGAIN** (呼び手は `sys_yield` / タイマで再試行)。着いていれば status と本文長 |
| 210 | `host_read` | `i32 h, void *buf, u32 cap` | 読んだ長さ / 0 = EOF / `AGAIN` / 負 | L2 ストリームから最大 cap。データ未着で EOF でもなければ AGAIN。消費した分だけ credit が回復する (§3 の思想: OS32 側のメモリは buf 分だけ) |
| 211 | `host_write` | `i32 h, const void *buf, u32 len` | 受け付けた長さ / 負 | `PRINT DATA` / `CLIP PUT` / `PUT` の本文。内部で 1400B 以下の REQUEST に分割し ACK を待たずに次を送る (ホスト側は余裕あり)。TX スロットが埋まっていれば AGAIN |
| 212 | `host_close` | `i32 h` | 0 | 未読ストリームを捨て、ハンドルを返す。`exec_reclaim_owned` で **owner 回収** (con_sink / launch と同じ位置に `host_owner_exit`) |

- エラー写像: 送信失敗 `OS32_ERR_IO` (-1)、引数不正 `INVAL` (-9)、未対応 `NOSYS` (-10)、待ち `AGAIN` (-14、K7 で取得済み)、リンク断 (HELLO 再同期中) `STALE` (-11)。固有番号は取らない。
- **カーネル内の駆動**: `link_poll()` は既に IRQ5 + 100Hz ウォッチドッグで回っている。`host_status` / `host_read` は状態を読むだけで、待たない。GUI 配下のアプリは T8 D8 のポーリング型 (WAIT_POLL) と同じ作法で待つ (`sys_yield` → 再試行)。CUI のコマンドは `sys_halt` ループ。
- 同期版 `link_service_get()` は自己試験用に残す (KAPI にはしない)。
...
- 単位: 1 ジョブ = 1 文書。ジョブ id はホストが振る (再起動でリセットしてよい)。
...
| **N2 (ホスト)** | `host_agent.py` に `PRINT OPEN/DATA/CLOSE/STATUS`、`CLIP GET/PUT`、`PUT /file/`、`--root` / `--allow-put` / `--to-file`、pywin32 は任意依存 (無ければ to-file) | — | Python 単体試験 (スプール、分割の seq 欠落 409、許可リスト外 403) |
...
3. **LGY-98 を既定ビルドに入れるか**: 現状は `LGY98=1` の別ビルド。Host Services を製品機能にするなら既定に入れる (カーネル +20KB、未検出なら無効化されるだけ)。推奨: N3 受入後に既定へ。
