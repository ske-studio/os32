...
## 追補 2 (同日夕): FD の読み出し高速化 + ビープの修正

午後の回の結果: 桁ズレは直った (`gfxmode auto` のまま)。シリアルは FD を入れ直した後に疎通が戻った (ソフト要因は見つからず)。**シリアル通信中にビープが鳴り続けた** (0035h の全体書き、→ 修正)。**FD 起動のフォント読み込みが 1 分以上** (シーク完了 IRQ の取りこぼしで 1.5 秒待ち × 毎セクタ + 1KB ごとの開き直しで 4 トラック巡回、→ 修正、NP21/W で 187 秒 → 3 秒)。

**実機に書くのは生イメージ** (`os32_boot.img` = 2HD 1232KB、`os32_boot144.img` = 1.44MB)。`.d88` はエミュレータ用 (ヘッダ付き)。

新しい FD で見るもの:

| # | 見るもの | 期待 |
|---|---|---|
| 1 | 起動の時間 (電源〜シェル) と、フォント読み込みの後の 3 行 `[fdc] font: seek= ... tmo=` / `multi= ...` / `cache ... idle=` | tmo=0、seek は数十回以下。写真を |
| 2 | ノートからシリアルで `ver` など | ビープが鳴らない |
| 3 | 桁ズレ・キーボード | 引き続き問題なし |
