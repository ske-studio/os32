...
**まず「ステージA: エンジン拡張」を完了させる(M0)。** ゲーム移植(ステージB)はその後に着手する。

### 最優先(ステージA / 別冊 `ENGINE_EXTENSION_PLAN.md` に従う)

1. **`libos32turn` を実装**(`programs/libos32turn/` 新設 + `Makefile`/`docs/tasks/libturn/` 登録)→ `turn_test.c` 合格。【A-1】
2. **`libos32rpg` を実装**(EXP曲線/成長/状態異常tick/リボーン)→ `rpg_test.c` がDOS `player.c` のゴールデン値と一致。【A-2】
3. **`libos32save` を実装**(領域登録→magic/version/checksum 書込)→ `save_test.c` 合格。【A-3】

### 並行で準備可(ステージBの仕込み・検証基盤)

4. **ホスト側ゴールデン値生成の整備**: DOSロジック(または `docs/19` のテスト)を gcc でビルドし、`player.c`/`battle.c`/`village.c` の期待値を出力 → A-2/ステージBの照合に使う。
5. **`gen_board_db.py` を実マップへ拡張**: `map_data.c` の全マス・接続・区画を `board.db` に投入(Phase 1 の仕込み)。

M0(エンジン拡張)達成後、ステージB Phase 0(`programs/game/` 雛形)へ進む。最初の可視成果(盤上をキャラが動く)は Phase 1 完了時。
