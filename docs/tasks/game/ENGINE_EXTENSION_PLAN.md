...
- [ ] `libos32turn` 実装 + `turn_test.c` 全合格(4人ローテ・週境界・スキップ・最大ターン)
- [ ] `libos32rpg` 実装 + `rpg_test.c` 全合格(EXP/成長/状態tick/リボーンが DOS `player.c` と一致 or 分布一致)
- [ ] `libos32save` 実装 + `save_test.c` 全合格(往復/破損/version)
- [ ] 3libが `Makefile` でビルドされ、`gen_rpg_db.py` が `assets/rpg.db` を生成
