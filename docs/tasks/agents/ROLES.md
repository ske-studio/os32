...
| 5 | **ローカルモデル直接方式** (`tools/review_local.py`) | スキル **`os32-local-review`**。対象を行番号つきで埋め込み 1 回で返させる。**opencode は使わない** | **使える (2026-09-16 に実測)**。指摘はすべて実在・作り話 0、既知 9 件中 5 件。A100 で 39 秒。**Q4 と Q8 で見つける欠陥が違う**ので重要な回は両方回す |
