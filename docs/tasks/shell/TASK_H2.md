...
戻り値: 公開済みで段 3〜5 まで通れば `EXT2_OK`。公開済みだが後始末が落ちたら `EXT2_ERR_IO`
...
| X3b | 対象フィールドが**前半**セクタ / **後半**セクタの配置で、前半だけ成功 | 前半配置 = 公開済み、後半配置 = **未公開**。未公開で段 5 に進んで `D` を解放しない (往復 2 所見 2) |
...
| A14a4 | 段 2 の書き込みが失敗し、読み直しも失敗 (不明) | マウントが書き込み禁止になり、段 3〜5 を実行しない。`D` を解放しない。hsync は `replace_unknown` |
