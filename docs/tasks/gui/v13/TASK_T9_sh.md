...
| **S7** | Start → CUI mode: 端末 + sh + kbd_echo が生きた状態から `appslot_reclaim_count` 5 → 8 (`last_reclaim_id` 2 = 端末が最後 = 末尾から)、CUI シェル応答 (`ver` API v49)、regress 6 本 (テスター、obs は §17 の下)、常駐 `shell.bin` は継承修正 (R6/R7/I1〜I6/C-1) 以外の .o がバイト一致 (S の突合)、`SHELL_AS_APP` の `exec_run` 参照 0 件 | 継続 (v86 -t と regress obs を確認中) |
