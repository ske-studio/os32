...
| **G6** 8MB | `ram-8mb` で `gui_demo` を Run → 「Launch failed (… out of memory)」、`ring3_transition_count` 0。CUI から `gui_demo` を実行すると `[shlib] no memory for 4 data pages` / `Error: shlib data attach failed (out of memory)`、`exec_sbrk_tier_last` 1 (3 領域は張れた後に共有ライブラリ .data 4 ページが取れない) | **不合格 → K7** (カーネル小票、TASK_K5B_kernel.md) |
