...
| G7 | 切替点 | **生存アプリ間の実行切替**は `OP_WAIT` の中でだけ起きる (カーネルのカウンタで証明)。起動 (`exec_start`)・終了・fault・kill に伴う CR3 の遷移は**別勘定**で数え、G7 の失敗扱いにしない (独立レビュー 2026-09-10 の指摘) |
...
| turn は 1 ラウンド 1 回 | `turn_used` | park 中のアプリは 1 ラウンドに 1 回しか選ばれない。全員が使い切ったら一斉にクリアして次のラウンド |
...
#### D11-3a. 有界性 (これが飢餓しない根拠)

1 本の turn が続く `OP_WAIT` は最大 `INPUT_STREAK_MAX + 1` 回
(据え置き 4 回 + park する 1 回)。turn は 1 ラウンドにつきアプリごと 1 回しか
回ってこないので、1 ラウンドは最大 `MAX_APPS` turn。したがって

> **ready なアプリは必ず `MAX_APPS × (INPUT_STREAK_MAX + 1)` = 20 回の
> `OP_WAIT` 以内に走る。**

`focus` の優先も `turn_used` の下に置いたので、フォーカスを握ったままのアプリが
ラウンドを独占することはできない (初版の穴)。ホストモデルのケース 15 は
レビュアーの反例そのもの (A が毎周自分に `Focus` を湧かせ、B は `Paint` 待ちで
park 中) で、**B は 10 回目の `OP_WAIT` で走る** (アプリ 2 本のときの実測値。
上の式の上限 20 の内側)。
...
`tools/tests/multiapp_model_host.c` にケース 12〜15 (計 18 検査) として入れた。
...
| 15 | **レビュアーの反例そのもの** — A が毎周自分に `Focus` を湧かせ、B は `Paint` 待ちで park 中。(a) park は必ず起きる (b) B は必ず走る (c) B は `MA_STARVE_BOUND` = 20 回の `OP_WAIT` 以内に走る (実測 10 回目) |

模型の定数 `MA_INPUT_STREAK_MAX` (= `MA_MAX_APPS` = 4) と
`MA_STARVE_BOUND` (= `MA_MAX_APPS × (MA_INPUT_STREAK_MAX + 1)` = 20) が
D11-3a の式そのもので、ケース 15c がそれを検査している。

RED→GREEN の記録は
[`tools/tests/multiapp_model_tdd.md`](../../../../tools/tests/multiapp_model_tdd.md) の
回 9 / 回 10 (D11 初版) と **回 11 (RED: 反例で park 0 回) → 回 12 (GREEN: 78 検査 ALL PASS)**。
...
- 1 ラウンドの turn を使い切ったあと、`turn_used` を一斉クリアした直後に
  **直前に走ったアプリがもう一度選ばれうる** (実測でも B は 10 回目 = park 2 回目で
  走っている)。上限 20 の内側なので規則としては満たしているが、handover を
  1 turn 速める余地はある (「クリア時に `last_run` の turn だけ使用済みで残す」)。
  複雑さに見合うかは G9 の実測で判断する。
