...
> **注記 (実装レビュー 往復 2、2026-09-13)**: 下の数値は往復 1 の修正までのもの。
> 往復 2 で `abs_path_buf` (256B) が増え、末尾判定を SQLite に委ねて自前の表が
> 消えた。**修正版の実数は K1 の clean build 後に再測する** (コーダーは `make` を
> 行えないのでここでは測れない)。増える .bss は解決後の絶対名 1 本 (+256B) だけで、
> `kmalloc` / exec_heap / アプリ帯は 1 バイトも減らさない。
...
| `paging.o` | 5328 → **5464** (+136) | 49215 で不変 | `paging_addrspace_pte_flags` のみ (`PDE.PS` の明示拒否を含む) |
