...
| PEGC 構成 | `gfxmode pegc` + リセットでも `hal_test` は pc98 のまま — K6-RAM 以後 `sys_mem_kb` が RAM 上端 (15MB 構成で 17MB) になり、`backend_pegc.c:219` の「上端 > 0xF00000 なら不可」が常に落ちる (15〜16MB は RAM でない穴なのに)。**K6-3 として修正中**。修正後に PEGC の F1 を回す | 未実施 (K6 回帰) |
