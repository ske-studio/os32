...
- カーネルは int 0x80 の入口で gshell が登録したフック `gui_pump()` を**上限付きで**呼ぶ
  ...
  入力を失わない。
- 純粋な計算ループの間は効かない。ISR 側の強制脱出キー (CTRL+STOP) で
...
- `open_modal(spec) -> DialogId`: WM が入力の宛先をそのダイアログに限定する。親は `Paint` を
  受け続ける (再描画は止まらない)。
  **例外 (2026-09-10 追記)**: 限定されるのは**モーダルが開いてから始まった入力**である。
  開く前にアプリへ配った `Button` 押下と対になる**離し**は、モーダル中に起きてもその押下を
  受けたアプリへ届く (WM は押下の配送先を左右ボタンごとに捕捉し、離しは同じ相手へ返す)。
  押したままダイアログを開くアプリでも押下と離しが必ず対になる。モーダル中に新しく
  ダイアログ上で押した分は従来どおりダイアログだけのもの。
...
2026-09-18 (穴 H13 の後始末。ユーザ承認。**末尾追記のみ**):
1. ジャンプ表 118 (`os32gui_w_focused_in`) を追記、`nfunc` 118 → 119。
   引数は **WindowId**。`GUI_PROTO_VERSION` は据え置き (`nfunc` の照合で守られる)。
2. **既存の 77 (`os32gui_w_focused`) は意味を変えない。** 引数は窓スロットの添字のまま。
   意味を変えると、添字を渡している呼び手 (単一窓なら 0 が偶然当たる) が黙って壊れる。
   **77 はアプリからは使えない** (添字を引く口が無い) ので、アプリは 118 を使う。
3. 背景: アプリがフォーカスを問い合わせられず自分で追うしかなかったことが、
   穴 H13 (**文字は入るのにカーソルキーだけが死ぬ**) を踏む前提になっていた。
   [`TASK_EDIT_GUI.md`](TASK_EDIT_GUI.md) §9 / §12。
