...
- **`OP_WAIT` が戻る条件**: リングに未読がある、**導出型が保留中** (損傷あり / Configure 未通知
  / 期限切れタイマ) のいずれか、または期限。「`Paint` 待ちだけが残る」状態で眠らない。
...
- **保持できる件数と溢れ** (2026-09-05 再改訂。「欠落しない」保証は取り下げ): アプリが
  ...
  (1) リング内の `Pointer` を落とし、(2) それでも無ければ取り込みを止めて新しいものを捨て、
  ヘッダの `dropped: u16` を増やして `OVERFLOW` を立てる。カーネル側で捨てられた打鍵は
  数えられない (仕様上の限界として明記)。アプリは次の `OP_POLL` で `OVERFLOW` と `dropped` を
  受け取り (受け取った時点で WM が消す)、入力状態を未知として扱う (押しっぱなしの前提を
  捨てる。押し上げは元々届かないので `kbd_is_pressed` で読み直す)。長い処理の途中で
  `OP_POLL` を挟むのは、この上限に当たらないための作法 (U3)。
...
`window: u16` (2、ウィンドウ ID の index 部) / `serial: u16` (4、入力系のみ。他は 0)。
`window` に generation を載せない代わりに、**index の再利用を遅らせる規則**で古いイベントが
新しい窓に届くのを防ぐ (2026-09-05 再改訂。「生存している窓にしか配送しない」だけでは、
一括取得した後に窓 A を破棄して同じ index で B を作ると残りの A 宛が B に届く):
- WM は `destroy_window` された index を**所有者の次の `OP_POLL` が戻るまで再利用しない**
  (検疫)。破棄時点でリングに残っている A 宛の項目は `kind = NONE` に書き換える。
- クライアントライブラリは、自分の表で破棄済みになったハンドルの index 宛イベントを
  (取得済みの配列に残っていれば) 捨てる。
一括取得 → 処理中に破棄 → 同じ処理中に作成、の順でも新しい窓は別の index を得る。
...
- **文字入力は `Text` として別に届ける** (Wayland の wl_keyboard と text-input の分離):
  ...
  10B ずつ UTF-8 境界で区切り、続きがあれば `more=1` (U2 の配置)。
...
3. **リング満杯**: 印字可能キーを 60 回 (= 120 件) 連打してアプリが `OP_POLL` を呼ばない →
   全部届く。200 回 → 先頭 64 打鍵 + カーネル待ち行列 32 打鍵の範囲だけ届き、`OVERFLOW` と
   `dropped ≥ 1`、以後の取り込みは `head` が進むと再開する。
