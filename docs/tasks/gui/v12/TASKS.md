...
**現在の状態**: 2026-09-07 に main へマージ済み (`d739494`、親 `0b3286c` / `69ccb23`)。
以下の「レビュー待ち」「マージ待ち」は当時の記録。マージ確認はローカル Git の履歴に基づき、
今回の記録整合ではゲスト試験を再実行していない。9801 の v11 台本で Help の閉じるクリックが
座標ずれで外れる制約は、最終検証記録のとおり残る。
