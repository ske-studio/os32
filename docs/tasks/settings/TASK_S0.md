...
状態: **設計 第 2 版 (Codex 往復 1 の blocker 12 件を反映、往復 2 待ち)**。決裁: [S0_PLAN_2026-09-13.md](S0_PLAN_2026-09-13.md) §3 (2026-09-13、4 点承認 + 初期値はビルド毎に生成しインストーラに添付)。
...
- `db_error_code` は **診断専用の lookup** を使う (`slot_get` は解放済み / 隔離 slot を拒否するので使わない)。保持する診断は open 失敗 (owner 別)、prepare / bind / step / exec / finalize / close の最初の失敗。成功時 0、無効 handle は `SQLITE_MISUSE`。
- 新 handle には既存の `db_step` (ROW=1 / DONE=0 / 失敗=-1、DONE で自動 finalize) / `db_finalize` / `db_close` をそのまま使う。反復は再 prepare (reset API は足さない)。
- ポインタ引数の検証 (**B1**): 既存の `argptr` マスク + `ring3_ptr_ok` は**先頭番地だけ**を見るので、v50 の wrap は自前で範囲を検査する。`kapi/kapi_db.c` に `db_user_range_ok(const void *p, u32 len)` (= `p != NULL`、`p + len` が overflow しない、`[p, p+len)` の**全ページ**が `ring3_ptr_ok` の許可帯) を置き、`db_bind_text` / `db_bind_blob` は `length` で、`path` / `sql` は「上限バイトまでの範囲内で NUL を探す」(NUL が見つかるまでの各ページを検査、上限内に NUL が無ければ拒否) で検証してからカーネル側の固定バッファへコピーする。コピー先は slot ごとではなく **1 本の静的スクラッチ** (sql 1024B、path 256B、text 256B (255 + NUL)、blob 4096B) — bind は `SQLITE_TRANSIENT` で SQLite 側が写すので、次の呼び出しで上書きされてよい。SQLite に呼び手のポインタを保持させない。受入 K2 に不正範囲 (許可帯の末尾 -1 から 2B、`length` 負、`ptr + length` の overflow、NUL 無しの path) の拒否ケースを含める。
...
- 新規 `tools/deploy_protect.py`:
  - `resolve_dest(root, guest_path, host_src) -> str`: **実際の最終コピー先**を確定する (ディレクトリ指定 `guest: /etc/` は basename を補う、`..` / 連続 `/` / `./` を畳む)。root (マウントした NHD のツリー / HostDrv のルート) の外へ出る結果は例外 (拒否)。
  - `is_protected(root, dest) -> bool`: `dest` を **OS の `realpath`** で解決し (symlink / hardlink 先まで辿る)、root 内でなければ拒否、解決先の親が `<root>/etc` (大文字小文字を無視して比較) かつ basename が保護対象名 (同じく無視) なら真。加えて **保護対象の実体を指す別名** (解決先の `st_ino`/`st_dev` が `<root>/etc/settings.db*` のいずれかと一致) も真 (hardlink 対策)。
  - 判定は「作成 / 切り詰め / 削除 / rename の**直前**」に、確定した最終パスで行う。
- 適用点 (**すべて**、B3): `tools/nhd_deploy.py` の `do_sync` (cp、:670)、`do_sync_from_hostdrv` (:748)、`do_copy` / `do_copy_all` (:254)、`do_rm` (:310)、`remove_partial` (:585)、`prune_nhd` (:124); `tools/hostdrv_deploy.py` の `do_sync` (copy2、:157)、`do_clean` (:221/223 — `etc/settings*` を残す)、`prune_hostdrv` (:107); `tools/prune_stale.py` の削除。除外は明示ログ `protected: <path> (skipped)`。`nhd_deploy.py copy` / `copy-all` / `rm` の CLI からも保護対象は書けない (ホストのツールで settings.db を書く道を**作らない**)。
- **ゲスト側の `hsync`** (`userland/system/hsync.c:105`、HostDrv → NHD の再帰コピー、`O_CREAT|O_TRUNC`): 保護対象名 (`/etc/settings.db*`) を**コピーしない** (ログに skipped)。HostDrv に古い `etc/settings.db` が残っていても本 DB を切り詰めない。この 1 ファイルは S0-D の C 側の範囲。
- **NHD イメージ全体の配備** (B4): `do_deploy` は local → remote の全体コピーなので、ファイル単位の除外では稼働後の設定 (remote) が古い local で戻る。→ `nhd_deploy.py pull` が **stamp** (`build/nhd/os32.nhd.pulled`: remote の mtime + size) を残し、`deploy` は remote の現在の mtime + size が stamp と一致しない (= pull 後に remote が変わった / pull していない) とき**失敗**する (`--force` でだけ通す。従来の "自動 pull" も stamp を更新する)。`make deploy-nhd` はこの検査を通るときだけ全体を書く。
- **失敗の伝播** (B5): `do_sync` / `do_sync_from_hostdrv` の `sync` 戻り値 (:687 / :763) と `prune_stale.py` の rm / sync 失敗 (:124) を非ゼロ終了にし、後続 (NHD deploy) へ進めない。保護対象の**除外**は失敗ではない。
- 通常 deploy はDB や journal を作成・上書き・削除・rename しない。`NO_PRUNE` に依存しない。`/etc` を prune 対象に広げない。`do_clean` / `do_init` / `do_format` は明示の全消去操作として通常更新から分離し、本票では触らない (呼ぶ側の承認 [D2] に委ねる)。
- 例外: 明示 install (FDD / CD の `install.bin` / `cdinst.bin` がゲスト内で行うコピー) と `cfg init` (S2、ゲスト内) はツールの対象外。
- ホスト TDD (B12): temp dir + mock で (1) **保護対象の source を意図的に置く** fixture (HostDrv / manifest に `etc/settings.db` と `-journal` を置き、宛先に別内容の DB がある / 無い) で、前後の存在一覧と hash が不変、`protected:` が出る; (2) tsv だけの通常配備では `protected:` が出ず tsv が更新される; (3) `guest: /etc/` のディレクトリ指定、symlink / hardlink の別名、大文字名、`..`、sync-from-hostdrv、copy / copy-all / rm CLI、prune の各経路; (4) 失敗注入 (cp / sync / rm) で非ゼロ、保護対象への write 系呼び出し 0; (5) stamp 不一致で deploy が失敗、`--force` で通る。`sudo` / `mount` / 実配備は遮断。`make check` に登録 (PM)。
...
- ビルド統合 (B9): `build/assets.mk` の `$(BUILD_OUT)/settings.db` は **`FORCE` 依存** (ビルド毎に必ず生成、ユーザー決裁)。`all` / `assets-all` に加え、**`images/os32_boot.d88` と `packages` (`build/image.mk`) から直接依存**させる (clean 後の単独 `make iso` でも生成される)。`clean-assets` / `clean` で消す。`tools/mkpkg.py` はこのファイルの欠損を warning で飛ばさず**エラー**にする (パッケージ定義に `required: true` を足す、または欠損時は非ゼロ)。`make -j` の競合は依存で解決する (生成ターゲット 1 本に集約)。
...
| D1 | (a) HostDrv に**意図的に** `etc/settings.db` (別内容) を置いた状態で `make deploy` → ゲストで `hsync` → `make deploy-nhd` (pull 済み) | ログに `protected:` が出て、ゲストの `/etc/settings.db` の有無と内容 (hash) が前後で不変; (b) tsv だけの通常配備で `protected:` が出ず `/etc/settings.tsv` が更新される; (c) pull せずに `deploy-nhd` → stamp 不一致で失敗 |
