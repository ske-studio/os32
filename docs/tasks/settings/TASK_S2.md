...
状態: **設計 第 5 版 = 実装へ (ユーザー決裁 2026-09-13: 設定変更は OS 経由だけ、アプリは読みだけ / 第 4 版のまま実装に進み実装レビューで併せて見る)**。前提: S0-K (KAPI v50、`8bfe...`〜`a70df4f`)、S0-T (`/etc/settings.tsv` を通常配備、生成 DB は媒体だけ)、S0-D (通常配備は `/etc/settings.db*` を触らない)。決裁: [S0_PLAN_2026-09-13.md](S0_PLAN_2026-09-13.md) §3 (2 = 明示 `cfg init`、4 = S4 の最初の消費者は数キー)。
...
| **S2-W** | libos32gui の末尾追記 (**読み取り専用の** cfg get を GUI アプリへ公開、次の空きエントリから) | W | `userland/rust/libos32gui/src/*` (ffi / lib の表)、`sdk/rust/os32api` の宣言、`tools/check_gui_proto.py` の期待値 |
...
## 3. libos32gui への末尾追記 (S2-W) — 読み取り専用 (決裁 2026-09-13)

- アプリに公開するのは **get だけ** (設定変更は OS 経由のみ): 末尾 **2 本** `os32gui_cfg_get_int(scope, key, def)` / `os32gui_cfg_get_text(scope, key, out, cap)`。それぞれ **1 呼び出しの中で `cfg_open(&db, 0)` → `cfg_get_*` → `cfg_close`** を完結させ、接続を呼び手に渡さない (`CfgDb` を公開しない)。MISSING / CORRUPT / VERSION でも get の規則どおり (既定値 / NOTFOUND)。open が負を返したら get_int は def、get_text は負をそのまま返す。close の失敗は握りつぶさず get_text の戻りに `OS32_ERR_IO` 相当を返す (get_int は def)。
- ジャンプ表は `tools/mkshlib.py --check` が `.long os32gui_*` を抽出して本数を照合する (B11)。表に載せる名前は wrapper (Rust の `#[no_mangle] extern "C"` か C の thin wrapper) で、実体の `cfg_*` (C、`libos32cfg.a` を shlib にリンク) を呼ぶ。既存エントリは不変。表の実体は `userland/rust/libos32gui/src/shlib.rs` (現在 101 本) と SDK stub `sdk/rust/os32api/src/gui/stub.rs`、本数定数は両方を **103** に更新し、`mkshlib.py --check` が通ること (`check_gui_proto.py` は C/Rust の共有定数・構造体の検査で表の本数は見ない)。SDK の C ヘッダ (`sdk/include/os32/` の GUI 呼び出し表) にも 2 本を足す。
...
- **接続の直列化**: 「アプリが触ると同時接続になるのか」という問いに対する答えは「はい — 2 つのプロセスがそれぞれ接続を持った状態 (例: アプリの RW と gshell) が同時接続」。決裁は **設定変更は OS 経由 (gshell の設定 UI / `cfg` コマンド) だけ認める、アプリは読みだけ**。これで書き手は OS の 2 経路に限られ、いずれも open〜close を yield なしで 1 回の実行に収めるので接続は構造的に同時 1 本 (§1-1 (c))。KAPI 側の門 (v51) は作らない。
