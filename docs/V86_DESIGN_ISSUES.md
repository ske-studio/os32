...
### 1.6 シェル帯域と V86 バッキング RAM の重なり
`v86_mem.c` 冒頭コメント:
> シェル帯域 (0x300000-0x3FFFFF) をバッキングRAMとして転用する。
> V86モード中はシェルは待機中のため [...]
> Phase 1 では v86_test から呼ばれるため、シェルは未使用。

しかし現在の `vdos.c` は **シェル経由で呼ばれており**、Phase 1 の前提が崩れている。現行コードはシェル帯域 (0x300000-0x39FFFF) を **退避せずに 0 クリア** (`v86_mem.c:103`) する。シェルの heap や stack 上のデータが破壊される可能性大。

実害が顕在化していないのは:
1. シェルの動的データの多くが 0x9XXXX 帯のスタック上に乗っているため、「方法 C リマップ」で逃がされている。
2. バッキング RAM の I/O 読み出しが 0 でも見かけ上動く。

しかし vdos 終了後に shell の heap 経由参照が壊れた事例が出れば、ここが原因である可能性が高い。
...
4. **バッキング RAM 物理 0x300000 はゼロクリアされる**: V86 起動のたび。シェルの動的データを置いている場合は破壊される (§1.6)。
...
**Q. V86 終了後にシェルが暴走する**  
A. §1.6 のシェル帯域破壊が最有力。`v86_mem.c:103` の `kmemset(backing, 0, V86_BACKING_SIZE)` がシェルのデータを潰している。
...
| ★★★ | シェル帯域とバッキング RAM の重なり解消 (退避 or 移転) | §1.6 |
...
末尾: 本書は 2026-05-04 時点のソース (`rtk/friendly-dhawan-132179` ブランチ) を基に作成。
