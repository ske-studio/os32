...
> ⚠️ **共有メモリのアドレス解決について**:
> 共有メモリのアドレス (`MEM_SHM_BASE`) はカーネルのBSSサイズに応じて動的に決定されます。そのため、ユーザー空間（`libos32db` など）から共有メモリ（SQLite結果バッファ）にアクセスする際は、固定値 `0x381000` ではなく、外部プログラム起動時に渡される `kapi` ポインタを基準に `(u8 *)kapi + 0x2000` で動的に解決します。
...
| 共有メモリ | 0x381000 | KAPI直後 (`kapi + 0x2000`) | 264KB |
