...
/*                                                                          */
/*  使い方: `host_test [stale]`。合格は最後の "host_test: PASS n/n"。         */
...
int main(int argc, char **argv, KernelAPI *api)
{
    ...
    if (api->version < 51) {
        api->kprintf(0x41, "host_test: kernel is older than v51\n");
        return 1;
    }

    /* ---- (0) リンクが立つまで待つ (link_tick が HELLO を通す) ---------- */
    if (!wait_up(600)) {
        api->kprintf(0x41, "host_test: no host agent (host_open keeps returning STALE)\n");
        return 1;
    }
    ...
    api->kprintf(failed ? 0x41 : 0x21, "host_test: %s %d/%d\n",
