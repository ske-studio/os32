...
メモリの最低動作環境 8MB は CUI 向けです。GUI の必要 RAM は実測で別途定義し、
開発を 8MB に制限しません。設計対象と現在の実装上限は
[メモリ方針](docs/02_memory.md)を参照してください。
...
- **KernelAPI v42** — 184エントリの関数テーブルによる外部プログラムインタフェース
