...
レビュアー = Codex (`codex exec`、枯渇時は Fable サブエージェント)、テスター = ローカル AI (`tools/emu_agent/`、スキル `os32-local-ai`)。
**v1.4 から**: 実装 PM は別エージェント、Claude Code は設計者 + レビュアー (ROLES §0、入口は `docs/tasks/agents/HANDOVER_v14.md`)。
